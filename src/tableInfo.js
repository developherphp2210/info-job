import modal_start_job  from "./modal";
...
export function TableInfo(props) {
            ...
            response.map((ele,index) => {       
                return (
                    <tr key={index}>                        
                        ...
                        <modal_end_job show={show} hide={handleHide} value_time={utility.Create_Time(ora)} /></td>:<td>{ele.ora_fine}</td>} 
    ...
    const table = (
        <div className="col-xl-9 border border-primary rounded mb-3">
            ...
            <modal_start_job />
                {/* <modal_start_job show={show} hide={handleHide} value_data={utility.Create_Data_App(data_ora)} value_time={utility.Create_Time(data_ora)}/>                 */}
        </div> 
    )
